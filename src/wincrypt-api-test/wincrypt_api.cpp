...
VOID GenerateGUID(CString &strGUID)
{
    BYTE junk[20];
    ...
    strGUID.GetBuffer(41);
    HashToString(junk, strGUID.GetBuffer());
}
...
BOOL CalculateFileHash(TCHAR *path, BYTE *hash)
{
    ...
    CAtlFile file;
    HRESULT hr = file.Create(path, GENERIC_READ | GENERIC_WRITE,
...
BOOL CheckSignature(TCHAR *path, TCHAR *hexSignature, int signatureLength)
{
    ...
    signature = (BYTE*)new BYTE[signatureLength];
    // Convert TCHAR signature to byte array
    HexToByteArray(hexSignature, signatureLength, signature);
    signatureLength /= 2;
    
    HRESULT hr = updateFile.Create(path, GENERIC_READ | GENERIC_WRITE,
                                   FILE_SHARE_READ | FILE_SHARE_WRITE, OPEN_EXISTING);

    if (SUCCEEDED(hr)) {
        ULONGLONG fileLength = 0;
        updateFile.GetSize(fileLength);
        //fileContents = (BYTE *)Allocate(fileLength);
        fileContents = (BYTE*)new BYTE[fileLength];
