...
public:
	...
	//将生成的数据输出
	bool WritePatchFile(CAxCryptoMath* pKeyMath, DWORD dwType);
	//设置进度输出窗口
	void SetProgressWnd(HWND hWnd) { m_hProgressWnd = hWnd; }
	//终止正在压缩的工程
...
private:
	...
	char* ConvertNumber( DWORD num );
public:
	static char* CryptName( char* pName, BYTE len );
	static void CryptOffset( DWORD& dwOffset );
	static void CryptSize( DWORD& dwSize );
	static char* DecryptName(char* name, int len);
	static void  DecryptOffset(unsigned long& dwOffset);
	static void  DecryptSize(unsigned long& dwSize);
	static void DecryptHead( char* lpPacketHead, DWORD dwHeadLen );
	static void CryptZipFile( char* lpFileName );
};
