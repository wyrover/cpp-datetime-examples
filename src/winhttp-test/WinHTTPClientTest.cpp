// WinHTTPClientTest.cpp : Defines the entry point for the console application.
//
...
{
    // Set URL.
    WinHttpClient client(L"http://downloads.bitoptimizer.com/");
...
void ProgressTest(void)
{
    ...
    WinHttpClient client(L"http://downloads.bitoptimizer.com/", ProgressProc);
    client.SendHttpRequest();
    wstring httpResponseHeader = client.GetResponseHeader();
    wstring httpResponseContent = client.GetResponseContent();
}
